...
use p2p_node::{NodeCommand, P2PNode};
...
struct VpnState {
    ...
    node_task_handle: Mutex<Option<tauri::async_runtime::JoinHandle<()>>>,
...
async fn start_vpn(
    ...
    let ip_addr: Ipv4Addr = start_ip_str
        .parse()
        .map_err(|e| format!("IP格式错误: {}", e))?;
    let mask: Ipv4Addr = "255.255.255.0".parse().unwrap();

    // Create shutdown channel (Broadcast)
    let (b_tx, _b_rx) = broadcast::channel(1);

    // Store shutdown sender in state
    {
        let mut shutdown_tx = state.shutdown_tx.lock().unwrap();
        *shutdown_tx = Some(b_tx.clone());
    }
    ...
    // Store node task handle
    {
        let mut handle_lock = state.node_task_handle.lock().unwrap();
        *handle_lock = Some(node_handle);
    }
...
async fn set_system_proxy(enable: bool, port: u16) -> Result<(), String> {
    #[cfg(target_os = "windows")]
    {
        ...
        let _ = Command::new("powershell")
            .args(&[
                "-Command",
                r"$mk = 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Internet Settings'; $v = (Get-ItemProperty -Path $mk).ProxyEnable; Set-ItemProperty -Path $mk -Name ProxyEnable -Value $v",
            ])
            .creation_flags(CREATE_NO_WINDOW)
            .output();
...
async fn update_services(
    ...
    if let Some(sender) = sender {
        sender
            .send(NodeCommand::UpdateServices(services))
            .await
...
async fn stop_vpn(app: tauri::AppHandle, state: State<'_, VpnState>) -> Result<String, String> {
    ...
    if let (Some(tx), Some(handle)) = (tx, handle) {
        println!("Sending shutdown signal to VPN node...");
        let _ = tx.send(());

        println!("Waiting for VPN node task to finish...");
        // Wait for up to 5 seconds for clean exit
        use tokio::time::timeout;
        use std::time::Duration;
        if let Err(_) = timeout(Duration::from_secs(5), handle).await {
            eprintln!("VPN node task timed out during shutdown. Proceeding with cleanup.");
        }
        println!("VPN node task finished.");
        ...
        println!("Stop command issued, but VPN was not running.");
        // Clean up inconsistent state just in case
        {
            let mut current = state.current_ip.lock().unwrap();
            *current = None;
            let mut port = state.socks5_port.lock().unwrap();
...
async fn do_quit(app: tauri::AppHandle, state: State<'_, VpnState>) {
...
#[tauri::command]
async fn quit_app(app: tauri::AppHandle, state: State<'_, VpnState>) -> Result<(), String> {
    do_quit(app, state).await;
    Ok(())
}
...
fn main() {
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        if uid != 0 {
            ...
            match status {
                ...
                _ => {
                    eprintln!(
                        "Failed to get root privileges. Application may not function correctly."
                    );
    ...
    tauri::Builder::default()
        ...
        .manage(VpnState {
            ...
            node_task_handle: Mutex::new(None),
        ...
        .setup(|app| {
            ...
            let connected_i =
                CheckMenuItem::with_id(app, "connected", "已连接", true, false, None::<&str>)?;
            ...
            let rules_i =
                CheckMenuItem::with_id(app, "mode_rules", "规则", true, true, None::<&str>)?;
                    ...
                    match id {
                        "quit" => {
                            ...
                            tauri::async_runtime::spawn(async move {
                                ...
                                do_quit(app_handle.clone(), state).await;
                            });
                        }
                        "show" => {
                            if let Some(window) = app.get_webview_window("main") {
